...
/**
 * Full 105-key ISO layout (the ISO equivalent of Full 108 ANSI).
 *
 * Differences from ANSI Full 108:
 *   - Enter is tall (L-shape, 1.5u wide × 2u tall) instead of 2.25u wide
 *   - Left Shift is 1.25u (not 2.25u) with an extra <\| key next to it
 *   - Row 2 has the backslash key next to Enter instead of above it
 *   - Total key count: 105 (3 fewer than ANSI 108 — ANSI has 2 extra keys
 *     from the wide Enter + wide Shift; ISO has the tall Enter + extra key)
 *
 * Grid: ~25u wide × 6u tall (including nav cluster + numpad)
 */
export const full108Iso: LayoutDef = {
  id: 'full108-iso',
  name: 'Full 108 ISO',
  widthU: 25.5,
  heightU: 6,
  keys: [
    ...
    // ─── Row 2: TAB row (ISO: backslash moves here, above Enter) ───
    ...
    // ISO: no backslash here (moved to row 3); the 1.5u slot is empty
    ...
    // ─── Row 3: CAPS row (ISO: extra key after L, tall Enter is L-shape) ───
    ...
    { id: 'K_BSISO',legend: '\\',  region: 'alpha',  x: 1.25, y: 4, w: 1, h: 1, sculptRow: 1 },  // ISO extra backslash
